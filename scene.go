...
func (scene *Scene) Render(parallelCount int) (Pixels, chan struct{}) {
	...
	go func() {
		...
		totalRaysPerPixel := 0
		for i := range scene.raysPerPixel {
			totalRaysPerPixel += scene.raysPerPixel[i]
		}
		...
		// loop for each phase
		for rppi := range scene.raysPerPixel {
			...
			for c := 0; c < parallelCount; c++ {
				...
				go func() {
					...
					for ps := range pixelsToProcess {
						...
						for i := range ps {
							if ps[i].raysPerPixel > 0 {
								col := ps[i].color.Scale(1.0 / float64(ps[i].raysPerPixel))
								pixels[ps[i].k] = col.PixelValue()
							}
						}
						
						// render every pixel in the line
						for i := range ps {
							pixels[ps[i].k] = scene.render(rnd, ps[i], scene.raysPerPixel[rppi])
						}
					}
					wg.Done()
				}()
			...
			accumulatedRaysPerPixel += scene.raysPerPixel[rppi]
			...
			fmt.Printf("Processed %v rays per pixel in %v. Total %v in %v. ERM %v\n", scene.raysPerPixel[rppi], time.Now().Sub(loopStart), accumulatedRaysPerPixel, totalTimeSoFar, erm)
		}
